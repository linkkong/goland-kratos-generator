...
class KratosToolWindowFactory : ToolWindowFactory {
    private val LOG = Logger.getInstance(KratosToolWindowFactory::class.java)
    
    override fun createToolWindowContent(project: Project, toolWindow: ToolWindow) {
        try {
            // 记录当前运行环境信息
            LOG.info("初始化工具窗口，环境: ${IDEVersionHelper.getVersionInfo()}")
            ...
            // 添加环境信息
            val envStyle = SimpleAttributeSet(normalStyle)
            StyleConstants.setItalic(envStyle, true)
            StyleConstants.setForeground(envStyle, JBColor.GRAY)
            doc.insertString(doc.length, "\n当前环境: ${IDEVersionHelper.getVersionInfo()}\n", envStyle)
